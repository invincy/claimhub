...
document.addEventListener('DOMContentLoaded', function() {
    ...
    initDB().then(() => {
        ...
        if (toolsPanel) {
            ...
            var requirementsBtns = document.querySelectorAll('.requirements-btn');
            ...
            // 3. Requirements Button Group Logic
            ...
            if (requirementsBtns && requirementsBtns.length) {
                requirementsBtns.forEach(btn => {
                    btn.addEventListener('click', function() {
                        requirementsBtns.forEach(b => b.classList.remove('active'));
                        this.classList.add('active');
                        showRequirement(this.dataset.type);
                    });
                });
                // Activate first button by default
                var first = requirementsBtns[0];
                if (first) { first.classList.add('active'); showRequirement(first.dataset.type); }
            }
...
calculateBtn?.addEventListener('click', async function() {
    ...
    // Truncate any fractional rupee (e.g., 435.6 -> 435)
    const annualPremium = Math.floor(per1000AfterSA * saThousands);
    // Compute modal premium: apply mode multiplier then round using 0.5 threshold
    const modalPremiumRaw = annualPremium * multiplier; // may have decimals
    const modalPremium = Math.round(modalPremiumRaw); // round .5 up
    ...
    let survival = 0;
    if (String(plan) === '179') {
        // For money-back plans, payouts depend on how many years premiums are paid (paidYears).
        // Count full 4-year multiples strictly less than paidYears.
        const pct = Math.abs(paidYears - 16) < 1e-6 ? 0.15 : 0.10; // special higher pct only when paidYears == 16
        for (let yr = 4; yr < paidYears; yr += 4) {
            survival += pct * saRupees;
        }
    }
    if (String(plan) === '174') {
        // Count full 4-year multiples in paidYears. Examples: 13 -> 3 (4,8,12); 7 -> 1 (4).
        for (let yr = 4; yr < paidYears; yr += 4) {
    ...
    const breakdown = [
        ...
        `Total over term: ₹${modalPremium.toFixed(0)} × Inst./yr: ${inst} × Policy term: ${policyTerm} = ₹${totalPremium.toFixed(0)}`,
...
var maturityBtn = document.getElementById('calculateMaturityBtn');
maturityBtn?.addEventListener('click', function() {
    const maturityPremium = parseFloat(document.getElementById('maturityPremiumInput')?.value);
    const paidYears = parseFloat(document.getElementById('maturityPaidYearsInput')?.value);
    const sa = parseFloat(document.getElementById('maturitySaInput')?.value);
    const plan = document.querySelector('input[name="maturityPlan"]:checked')?.value;
    const mode = document.querySelector('input[name="maturityMode"]:checked')?.value;
    if (Number.isNaN(maturityPremium) || Number.isNaN(paidYears) || Number.isNaN(sa) || !plan || !mode) {
        showToast('Please fill all maturity fields with valid values.');
        return;
    }
    ...
    // dab deduction: ₹100 per annum distributed across instalments
    const dabPerInstall = 100 / inst;
    // adjusted premium per installment after dab deduction
    const adjustedPremiumPerInstall = Math.max(0, maturityPremium - dabPerInstall);

    // number of installments actually paid (round to nearest integer to avoid fp issues)
    const installmentsPaid = Math.round(paidYears * inst);

    const totalPaid = adjustedPremiumPerInstall * installmentsPaid;

    const saRupees = sa;
    // Survival / money-back paid (sum of payouts already made during paidYears)
    let survival = 0;
    if (String(plan) === '179') {
        const pct = Math.abs(paidYears - 16) < 1e-6 ? 0.15 : 0.10;
        for (let yr = 4; yr < paidYears; yr += 4) {
            survival += pct * saRupees;
        }
    }
    ...
    const maturity = Math.round(totalPaid - survival);

    // Show results
    const panel = document.getElementById('maturityResult');
    if (panel) panel.classList.remove('hidden');

    document.getElementById('maturityTotalPaid').textContent = `₹${Math.round(totalPaid)}`;
    document.getElementById('maturitySurvival').textContent = `₹${Math.round(survival)}`;
    document.getElementById('maturityAmount').textContent = `₹${maturity}`;

    const breakdown = [];
    breakdown.push(`Premium per install: ₹${maturityPremium.toFixed(2)}`);
    breakdown.push(`Dab per install: ₹${dabPerInstall.toFixed(2)} ⇒ Adjusted per install: ₹${adjustedPremiumPerInstall.toFixed(2)}`);
    breakdown.push(`Installments paid: ${installmentsPaid}`);
    breakdown.push(`Total paid (after dab): ₹${Math.round(totalPaid)}`);
    breakdown.push(`Survival/money-back paid so far: ₹${Math.round(survival)}`);
    breakdown.push(`Estimated maturity: ₹${maturity}`);

    document.getElementById('maturityBreakdown').innerHTML = breakdown.join('<br>');
});
...
const PLAN_174 = {
    ...
    "63": {"12":141.45,"16":null,"20":null}
        ...
        async function getTabularPremium(plan, age, term) {
            ...
            // --- Maturity Calculator Handler ---
            (function setupMaturityHandler(){
                const btn = document.getElementById('calculateMaturityBtn');
                if (!btn) return;
                btn.addEventListener('click', function () {
                    const plan = document.querySelector('input[name="maturityPlan"]:checked')?.value || '111';
                    const mode = document.querySelector('input[name="maturityMode"]:checked')?.value || 'YLY';
                    const saThousands = parseFloat(document.getElementById('maturitySaInput')?.value) || 0; // user enters SA in '000s
                    const sa = saThousands * 1000;
                    const premiumPerInstall = parseFloat(document.getElementById('maturityPremiumInput')?.value) || 0;
                    const paidYears = parseFloat(document.getElementById('maturityPaidYearsInput')?.value) || 0;

                    const instalPerYear = (mode === 'YLY') ? 1 : (mode === 'HLY') ? 2 : (mode === 'QLY') ? 4 : 12;
                    // Dab premium is proportional to Sum Assured: ₹100 per ₹100,000 of SA per year.
                    // sa is in rupees (e.g., 100000); dabPerYear = sa / 1000 (=> 100 for 100,000)
                    let dabPerYear = sa / 1000;
                    // Plan 111 has in-built dab — do not deduct extra dab for plan 111
                    if (String(plan) === '111') dabPerYear = 0;
                    const dabPerInstall = dabPerYear / instalPerYear;
                    const adjustedPremiumPerInstall = Math.max(0, premiumPerInstall - dabPerInstall);
                    const installmentsPaid = Math.round(paidYears * instalPerYear);
                    const totalPaid = Math.round(adjustedPremiumPerInstall * installmentsPaid);

                    // Money-back / survival logic per plan
                    let survival = 0;
                    if (plan === '179') {
                        // Plan 179 (16-year term): 15% of SA at end of 4th, 8th & 12th years
                        const payoutYears = [4,8,12];
                        const rate = 0.15;
                        payoutYears.forEach(y => { if (y < paidYears) survival += Math.round(rate * sa); });
                    } else if (plan === '174') {
                        // Plan 174 (20-year term): 10% of SA at end of 4th, 8th, 12th & 16th years
                        const payoutYears = [4,8,12,16];
                        const rate = 0.10;
                        payoutYears.forEach(y => { if (y < paidYears) survival += Math.round(rate * sa); });
                    }
                    ...
                    // Update UI
                    const resultWrap = document.getElementById('maturityResult');
                    const totalPaidEl = document.getElementById('maturityTotalPaid');
                    const survivalEl = document.getElementById('maturitySurvival');
                    const maturityEl = document.getElementById('maturityAmount');
                    const breakdown = document.getElementById('maturityBreakdown');
                    if (totalPaidEl) totalPaidEl.textContent = '₹ ' + totalPaid.toLocaleString();
                    if (survivalEl) survivalEl.textContent = '₹ ' + survival.toLocaleString();
                    if (maturityEl) maturityEl.textContent = '₹ ' + maturity.toLocaleString();
                    if (breakdown) {
                        breakdown.innerHTML = `Plan: ${plan} · Mode: ${mode} · SA: ₹${sa.toLocaleString()} · Paid Years: ${paidYears} · Installments paid: ${installmentsPaid}`;
                    }
